...
const CopilotContextProvider = ({ children }: { children: ReactNode }) => {
    ...
    const generateCode = async () => {
        try {
            ...
            const systemPrompt = "You are a code generator copilot for project named Code Sync. Generate code based on the given prompt without any explanation. Return only the code, formatted in Markdown using the appropriate language syntax (e.g., js for JavaScript, py for Python). Do not include any additional text or explanations. If you don't know the answer, respond with 'I don't know'."
            
            const code = await generateWithGemini(systemPrompt, input)
